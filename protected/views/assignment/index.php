...
<?php if(!empty($dataProvider->getData())){ ?>
	...
	<?php foreach ($assign as $v) { ?>
		<?php if($type == 1){ ?>
			<?php $cekTgs = StudentAssignment::model()->findByAttributes(array('student_id'=>Yii::app()->user->id,'assignment_id'=>$v->id)); ?>
			<?php if(empty($cekTgs)){ ?>
				<div class="col-md-4">
						...
						<table class="table table-user-information table-responsive">
							<tbody>
								...
								<tr>
									...
									<td>
										<?php 
											if($v->lesson->moving_class == 1){
												echo $v->lesson->grade->name; 
											}else{
												echo $v->lesson->class->name;
											}
								...
								<?php }else{ ?>
									...
									<tr>
										<th>Keterangan</th>
										<td>
											<?php echo CHtml::link('Belum Dinilai <span class="badge">'.$total.'</span>', array('view','id'=>$v->id, 'type'=>1),array('class'=>'btn btn-warning'));?>
										</td>
									</tr>
								<?php } ?>
							</tbody>
