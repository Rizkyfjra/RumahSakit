...
<section id="typography" class="break-after" style="margin-top:10px;">		
    ...
    <h4 style="margin: 0px;" >LAPORAN KEMAJUAN HASIL BELAJAR SISWA BULAN JANUARI</h4></td>
		</tr>
		<tr>
	...
	<table border="5" style="width:100%;border: 2px solid black;">
		...
		<?php
			...
			if(!empty($peluas1)){
				foreach ($peluas1 as $key => $rowpeluas1) {
					if(!empty($rowpeluas1['nilai-kd1']) && !empty($rowpeluas1['nilai-kd1_ket'])){
								
								if(!empty($rowpeluas1['nilai-kd1'])){
									$rowpeluas1['p-np'] = Clases::model()->predikat($rowpeluas1['nilai-kd1'],$kkm); 
								}

								if(!empty($rowpeluas1['nilai-kd1_ket'])){
									$rowpeluas1['p-nk'] = Clases::model()->predikat($rowpeluas1['nilai-kd1_ket'],$kkm); 
								}
		...
		<tr>
			...
			<?php } ?>
				...
				<?php if(!empty($rowpeluas1['nilai-kd1']) && $rowpeluas1['nilai-kd1']!="-" && !empty($rowpeluas1['nilai-kddescription']) ) { ?>
				<td align="left">
					<?php 
							if (!empty($rowpeluas1['p-np']) && $rowpeluas1['p-np']=="A") {
								echo "Memiliki kemampuan sangat baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription']));
							} else if(!empty($rowpeluas1['p-np']) && $rowpeluas1['p-np']=="B"){
								echo "Memiliki kemampuan baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription']));
							} else if(!empty($rowpeluas1['p-np']) && $rowpeluas1['p-np']=="C"){
								echo "Memiliki kemampuan cukup baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription']));
							} else if(!empty($rowpeluas1['p-np']) && $rowpeluas1['p-np']=="D"){
								echo "Memiliki kemampuan kurang dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription']));
							}?>
				</td>
			<?php }else{ ?>
				<td align="center">-</td>
...
<section id="typography" class="break-after" style="margin-top:10px;">		
	...
	<table border="5" style="width:100%;border: 2px solid black;">
		...
		<tr>
			...
			<?php if(!empty($rowpeluas1['nilai-kd1_ket']) && $rowpeluas1['nilai-kd1_ket']!="-") { ?>
				<td align="center"><?php echo $rowpeluas1['nilai-kd1_ket']; ?></td>
			<?php }else{ ?>
				<td align="center">-</td>
			<?php } ?>
			...
			<?php } ?>
				...
				<?php if(!empty($rowpeluas1['nilai-kd1_ket']) && $rowpeluas1['nilai-kd1_ket']!="-" && !empty($rowpeluas1['nilai-kddescription-ket'])) { ?>
				<td align="left">
					<?php 
							if (!empty($rowpeluas1['p-nk']) && $rowpeluas1['p-nk']=="A") {
								echo "Memiliki keterampilan sangat baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription-ket']));
							} else if(!empty($rowpeluas1['p-nk']) && $rowpeluas1['p-nk']=="B"){
								echo "Memiliki keterampilan baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription-ket']));
							} else if(!empty($rowpeluas1['p-nk']) && $rowpeluas1['p-nk']=="C"){
								echo "Memiliki keterampilan cukup baik dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription-ket']));
							} else if(!empty($rowpeluas1['p-nk']) && $rowpeluas1['p-nk']=="D"){
								echo "Memiliki keterampilan kurang dalam ".strtolower(implode(",", $rowpeluas1['nilai-kddescription-ket']));
							}?>
				</td>
			<?php }else{ ?>
				<td align="center">-</td>
...
<section id="typography" class="break-after" style="margin-top:10px;">		
	...
	<table border="0" style="width:100%;border: 0px solid black;margin-top: 0.5px;margin-bottom: 0.5px;">
		<tr>
			...
			<td width="30%">
				<p class="text-center">
					Bandung, <?php echo(DateToIndo(date('2019-02-20')));?><br/>
					Wali Kelas
					<br>
