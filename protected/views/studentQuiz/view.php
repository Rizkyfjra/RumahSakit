...
<?php 
$arr_questions = explode(",", $model->quiz->question);
// echo "<pre>";
            ...
            <?php if(!empty($jawaban)){ ?>
              <?php foreach ($jawaban as $key => $value) { ?>
                ...
                <tr>
                  ...
                  <td><?php $keynya = array_search($key, $arr_questions); 
                  echo $keynya + 1;
                  ?></td>
                  <td>
                    <?php if($soal->type != 2){ ?>
                    <div style="font-size: 20px">
                      <?php
                        ...
                        //strip_tags($text, '<p><a>');
                        $soal->key_answer = preg_replace("/\r|\n/", "", $soal->key_answer) ;
                        $soal->key_answer = strip_tags($soal->key_answer,"<img>") ;
                        $soal->key_answer = preg_replace('/\s+/', '', $soal->key_answer);
                        $soal->key_answer =  str_replace("/>","",$soal->key_answer);
                        $soal->key_answer =  str_replace(">","",$soal->key_answer);
                        ...
                        // echo htmlspecialchars($soal->key_answer);
                        // echo "</br>";
                        // echo htmlspecialchars($value); 
                         // if(strtolower($soal->key_answer) === strtolower($value)){
                        if($soal->key_answer === $value){
                          // echo $soal->key_answer;
                          echo "<span class='label label-success label-as-badge'>Benar</span>";
                        }else{
                          // echo $soal->key_answer;
                          echo "<span class='label label-danger label-as-badge'>Salah</span>";
                        }
                      ?>
                    <?php }else{ ?>
